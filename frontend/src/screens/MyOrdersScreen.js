...
function estadoLabel(estado) {
  if (estado === 'pendiente_comprobante') return 'Pendiente: subir comprobante';
  if (estado === 'comprobante_enviado') return 'Comprobante enviado';
  return estado;
}
      ...
      >
        <View style={styles.cardTop}>
          ...
          <View
            style={[
              styles.badge,
              item.estado === 'pendiente_comprobante'
                ? styles.badgeWarn
                : styles.badgeOk,
            ]}
          >
            <Text style={styles.badgeText}>{estadoLabel(item.estado)}</Text>
          </View>
        </View>
        <Text style={styles.cardMeta}>{date}</Text>
        <Text style={styles.cardTotal}>${Number(item.total).toFixed(2)}</Text>
        <Text style={styles.cardTap}>Toca para pagar o ver comprobante ▸</Text>
      </Pressable>
    );
  }
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <View style={styles.top}>
        <Text style={styles.hint}>
          Aquí ves tus pedidos. Si falta el comprobante, entra y súbelo desde la
          galería.
        </Text>
      </View>
      {loading ? (
